...
class Calculator(QWidget):
    def init(self):
        ...
        self.b_plus.clicked.connect(lambda:self._operator("+"))
        self.b_minus.clicked.connect(lambda:self._operator("-"))
        self.b_multiply.clicked.connect(lambda:self._operator("*"))
...
def result(self):
    ...
    if self.op=="/":
        if self.num_2==0:
            err=QMessageBox.critical(self,"error","warning error")
            err.exec()
        else:
            self.input.setText(str(self.num_1/self.num_2))
